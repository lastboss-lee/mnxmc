...
================================================================================
  ...
  으로 자동 선택됩니다.
    ...
    - 설치 후 dpkg 상태를 검사해 미설정(iU) 패키지가 남으면 실패로 보고합니다

  ※ 22.04 → 26.04 는 in-place 배포판 업그레이드가 아니라 **26.04 클린 설치**를
    전제로 합니다. 두 배포판의 공유 라이브러리가 t64 전환(libcurl4→libcurl4t64
    등)으로 갈라져 있어, 섞인 상태에서는 오프라인 세트만으로 의존성을 맞출 수
    없습니다. 26.04 장비에 22.04 세트를 넣었다가 패키지 80개가 제거된 사례가
    있습니다(현재는 OS 게이트와 apt --no-remove 가드로 차단).
