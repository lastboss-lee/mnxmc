...
class PerformanceMonitor(BaseModule):
    ...
    def format_cpu_bar(self, usage, bar_length=5):
        """CPU 사용률을 바 차트로 표현 (■■■■□)"""
        filled = int((usage / 100) * bar_length)
        empty = bar_length - filled
        
        bar = '■' * filled + '□' * empty
        return f"{bar} {usage:5.1f}%"
    ...
    def format_bytes_total(self, bytes_val):
        """총 바이트를 읽기 좋은 형태로 포맷"""
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                print("2. Detailed CPU Monitor")
                print("3. Enhanced Network Monitor") 
                ...
                if choice == '1':
                    ...
                    self.display_cpu_detailed()
                elif choice == '3':
                    self.display_enhanced_network()
                elif choice == '4':
    ...
    def display_enhanced_network(self):
        """대폭 강화된 네트워크 성능 모니터링 표시 - 상세 정보 포함"""
        print("\n" + "="*120)
        print(f"{'Enhanced Network Performance Monitoring':^120}")
        print("="*120)
        
        network_stats = self.get_enhanced_network_stats()
        
        if 'error' in network_stats:
            print(f"Network monitoring error: {network_stats['error']}")
            return
        
        # 활성 인터페이스와 비활성 인터페이스 분리
        active_interfaces = {name: stats for name, stats in network_stats.items() 
                           if isinstance(stats, dict) and stats.get('status') == 'UP'}
        inactive_interfaces = {name: stats for name, stats in network_stats.items() 
                             if isinstance(stats, dict) and stats.get('status') != 'UP'}
        ...
        # === 활성 인터페이스 실시간 성능 ===
        print("\n=== Active Interfaces - Real-time Performance ===")
        print(f"{'Interface':<12} {'Status':<8} {'Speed':<12} {'RX Rate':<12} {'TX Rate':<12} {'RX PPS':<8} {'TX PPS':<8} {'Utiliz%':<10}")
        print("-" * 110)
        
        for if_name, stats in active_interfaces.items():
            # 전송률 포맷
            rx_rate = self.format_bytes(stats['rx_rate_bps'])[:11]
            tx_rate = self.format_bytes(stats['tx_rate_bps'])[:11]
            
            # 사용률
            utilization = f"{max(stats['rx_utilization'], stats['tx_utilization']):.1f}%"
            
            print(f"{if_name:<12} {stats['status']:<8} {stats['speed']:<12} {rx_rate:<12} {tx_rate:<12} "
                  f"{stats['rx_rate_pps']:<8.0f} {stats['tx_rate_pps']:<8.0f} {utilization:<10}")
        
        # === 상세 인터페이스 정보 ===
        print(f"\n=== Interface Details ===")
        print(f"{'Interface':<12} {'MAC Address':<18} {'Driver':<12} {'Duplex':<8} {'MTU':<6} {'RX Errors':<10} {'TX Errors':<10}")
        print("-" * 100)
        
        for if_name, stats in active_interfaces.items():
            print(f"{if_name:<12} {stats['mac']:<18} {stats['driver']:<12} {stats['duplex']:<8} "
                  f"{stats['mtu']:<6} {stats['rx_errors']:<10} {stats['tx_errors']:<10}")
        
        # === 패킷 통계 및 에러 분석 ===
        print(f"\n=== Packet Statistics & Error Analysis ===")
        print(f"{'Interface':<12} {'RX Packets':<12} {'TX Packets':<12} {'RX Drops':<9} {'TX Drops':<9} {'Collisions':<11} {'Avg Size':<10}")
        print("-" * 100)
        
        for if_name, stats in active_interfaces.items():
            rx_avg = f"{stats['rx_avg_size']}" if stats['rx_avg_size'] > 0 else "0"
            tx_avg = f"{stats['tx_avg_size']}" if stats['tx_avg_size'] > 0 else "0"
            avg_size = f"{rx_avg}/{tx_avg}"
            
            print(f"{if_name:<12} {stats['rx_packets']:<12,} {stats['tx_packets']:<12,} "
                  f"{stats['rx_drops']:<9} {stats['tx_drops']:<9} {stats['tx_colls']:<11} {avg_size:<10}")
        
        # === 총 전송량 통계 ===
        print(f"\n=== Total Transfer Statistics ===")
        print(f"{'Interface':<12} {'Total RX':<15} {'Total TX':<15} {'Total Packets':<15} {'Multicast':<10}")
        print("-" * 80)
        
        for if_name, stats in active_interfaces.items():
            total_rx = self.format_bytes_total(stats['rx_bytes'])
            total_tx = self.format_bytes_total(stats['tx_bytes'])
            total_packets = f"{stats['rx_packets'] + stats['tx_packets']:,}"
            
            print(f"{if_name:<12} {total_rx:<15} {total_tx:<15} {total_packets:<15} {stats['rx_multicast']:<10}")
        
        # === 비활성 인터페이스 (간단히) ===
        if inactive_interfaces:
            print(f"\n=== Inactive Interfaces ===")
            inactive_list = []
            for if_name, stats in inactive_interfaces.items():
                status = f"{if_name}({stats['status']})"
                inactive_list.append(status)
            
            print(f"Down: {', '.join(inactive_list)}")
        
        # === 시스템 전체 네트워크 요약 ===
        total_rx_rate = sum(stats['rx_rate_bps'] for stats in active_interfaces.values())
        total_tx_rate = sum(stats['tx_rate_bps'] for stats in active_interfaces.values())
        total_errors = sum(stats['rx_errors'] + stats['tx_errors'] for stats in active_interfaces.values())
        
        print(f"\n" + "="*60)
        print(f"System Network Summary:")
        print(f"  Active Interfaces: {len(active_interfaces)}")
        print(f"  Total RX Rate: {self.format_bytes(total_rx_rate)}")
        print(f"  Total TX Rate: {self.format_bytes(total_tx_rate)}")
        print(f"  Total Errors: {total_errors}")
        print("="*60)

    def display_cpu_detailed(self):
        """상세한 CPU 모니터링 표시 - 고코어 시스템 지원"""
        print("\n" + "="*80)
        print(f"{'CPU Performance Monitoring (Per-Core)':^80}")
        print("="*80)
        ...
        print(f"\nTotal CPU Usage: {self.format_cpu_bar(cpu_total, 10)}")
        print(f"Load Average: {load_avg['1min']:.2f}, {load_avg['5min']:.2f}, {load_avg['15min']:.2f}")
        print()
        
        # 코어별 사용률 표시 - 4개씩 가로 배치 (고코어 시스템 대응)
        cores_per_line = 4
        print("Per-Core Usage:")
        print("-" * 80)
        
        for i in range(0, len(cpu_per_core), cores_per_line):
            line_cores = cpu_per_core[i:i + cores_per_line]
            
            for core_id, usage in line_cores:
                core_bar = self.format_cpu_bar(usage, bar_length=3)
                print(f"CPU{core_id:2d}: {core_bar}", end="  ")
            print()  # 줄바꿈
            
            # 너무 많은 코어일 경우 일시정지
            if i > 0 and (i + cores_per_line) % 32 == 0 and len(cpu_per_core) > 64:
                print(f"\n--- Showing cores {i+1}-{i+cores_per_line} of {len(cpu_per_core)} ---")
    ...
    def _show_overview(self):
        ...
        print(f"CPU Total: {self.format_cpu_bar(cpu_total, 10)}")
        
        # 메모리
        memory = summary.get('memory', {})
        if 'error' not in memory:
            print(f"Memory:    {memory['used_gb']:.1f}/{memory['total_gb']:.1f} GB ({memory['usage_pct']:.1f}%)")
        
        # 로드 평균
        load_avg = summary.get('load_avg', {})
        print(f"Load Avg:  {load_avg.get('1min', 0):.2f}, {load_avg.get('5min', 0):.2f}, {load_avg.get('15min', 0):.2f}")
        
        # 네트워크 요약
        network = summary.get('network', {})
        if 'error' not in network:
            active_count = len([name for name, stats in network.items() 
                              if isinstance(stats, dict) and stats.get('status') == 'UP'])
            total_rate = sum(stats.get('rx_rate_bps', 0) + stats.get('tx_rate_bps', 0)
                           for stats in network.values() if isinstance(stats, dict))
            print(f"Network:   {active_count} active interfaces, Total: {self.format_bytes(total_rate)}")
    ...
    def _show_processes(self):
        ...
        top_processes = self.get_top_processes(15)
        ...
        for proc in top_processes:
            if 'error' not in proc:
                print(f"{proc['user']:<10} {proc['pid']:<8} {proc['cpu']:<6} {proc['mem']:<6} {proc['command']}")
