...
"소유자 없는 파일 목록 (운영자가 직접 판단):
$(find / -xdev \( -nouser -o -nogroup \) -not -path "/proc/*" -not -path "/sys/*" 2>/dev/null | head -30 | sed 's/^/  /')
  
  조치 방법:
    chown root:root <파일>   # root에 귀속
    rm -f <파일>             # 불필요 파일 삭제 (운영자 판단 필요)
  
  전체 목록: find / -xdev \( -nouser -o -nogroup \) -not -path \"/proc/*\" -not -path \"/sys/*\" 2>/dev/null"
...
parse_summary() {
    ...
    # 한글 "양 호" / "취 약" 사이 공백 + ":" 구분 → ":" 기준 분할 후 숫자만 추출
    local F="$1"
    local G V N
...
if ! $DRY_RUN && [ "$CNT_APPLIED" -gt 0 ]; then
    ...
    if [ ! -x "$AUDIT_SH" ] && [ ! -f "$AUDIT_SH" ]; then
        ...
        # audit 재실행 (스크립트 디렉토리에서 실행되어 리포트가 거기에 생성됨)
